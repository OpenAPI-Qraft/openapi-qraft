...
1. `options?: { filters, select }`,
   ...
   - _If not provided_, all mutations matching to `<service>.<operation>` will be returned
     - `filters?: UseMutationStateFiltersByParameters | UseMutationStateFiltersByMutationKey`
       ...
       - `filters.parameters: Record<'path' | 'query' | 'header', Record<string, any>>` will be used for filtering mutations by parameters
     ...
     - `filters.predicate?: (query: Mutation) => boolean` will be used for filtering mutations by custom predicate
