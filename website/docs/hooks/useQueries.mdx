---
sidebar_label: useQueries
---

# useQueries(...)

The Hook enables you to concurrently execute multiple asynchronous data fetching operations.
Each query managed by `useQueries` can have its own loading, error, and data handling.
For more detailed information, explore the TanStack
[_useQueries(...) 🌴_](https://tanstack.com/query/latest/docs/framework/react/reference/useQueries) documentation.

```ts
const queries = qraft.<service>.<operation>.useQueries(
  {
    queries,
    combine,
  },
  queryClient
)
```

### Arguments

1. `{ queries, combine }`
   - **Required**, represents the options for the queries
   - `queries: QueryOptions[]`
     - **Required** queries to be executed
     - `queries.<number>.parameters: Record<'path' | 'query' | 'header', Record<string, any>>` will be used for the request
     - `queries.<number>.queryKey: QueryKey` will be used for the request instead of the `parameters`
       - `queryKey` and `parameters` are mutually exclusive
   - `combine?: (result: UseQueriesResults) => TCombinedResult`
     - **Optional**, a function to select the data from the mutation
2. `queryClient?: QueryClient`
   - **Optional** [_QueryClient 🌴_](https://tanstack.com/query/latest/docs/reference/QueryClient) to be used
   - _If not provided_
     - `QraftContext.queryClient` will be used if available
     - [_useQueryClient() 🌴_](https://tanstack.com/query/latest/docs/framework/react/reference/useQueryClient)
       result will be used as a fallback

### Returns
The `useQueries` hook returns an array with all the query results. The order returned is the same as the input order.

### Example

```tsx title="src/entityQueries.ts"
/**
 * Will execute the request two queries:
 * ###
 * GET /entities/3e3e-3e3e-3e3e
 * x-monite-version: 2023-09-01
 * ###
 * GET /entities/5c5c-5c5c-5c5c
 * x-monite-version: 2023-09-01
 **/
const entityQueries = qraft.entities.getEntities.useQueries({
  queries: [
    {
      parameters: {
        header: {
          'x-monite-version': '2023-09-01',
        },
        path: {
          entity_id: '3e3e-3e3e-3e3e',
        },
      },
    },
    {
      parameters: {
        header: {
          'x-monite-version': '2023-09-01',
        },
        path: {
          entity_id: '5c5c-5c5c-5c5c',
        },
      },
    },
  ],
  combine: (results) => results.map((result) => result.data),
});
```
