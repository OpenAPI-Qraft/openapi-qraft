...
Use the no-argument client as a static hooks client outside components. When you need cache methods in callbacks, read the Context value with `useContext` and create a method-only client with `createAPIClient(apiContext)`.
