...
- **Flexible configuration** - Change `queryClient`, `requestFn`, or `baseUrl` via Context without recreating the static hooks client

## Type Surface

Context-based clients have two different TypeScript surfaces:

- `createAPIClient()` uses the generated React Context and exposes hook callbacks, such as `useQuery`, `useMutation`, `useIsFetching`, plus key helpers. It does not expose imperative `QueryClient` methods like `getQueryData`, `setQueryData`, `cancelQueries`, or `invalidateQueries`.
- `createAPIClient(options)` uses the provided `queryClient`, `requestFn`, and `baseUrl` directly and exposes method callbacks. It does not expose React hooks.

Use the no-argument client as a static hooks client outside components. When you need cache methods in callbacks, read the Context value with `useContext` and create a method-only client with `createAPIClient(apiOptions)`.
...
--create-api-client-fn createAPIClient \
  ...
  callbacks:useQuery,useMutation,getQueryKey,getInfiniteQueryKey,setQueryData,getQueryData,cancelQueries,invalidateQueries
```
...
use `useContext` to get options and create a method-only client instance.
Keep hooks on the static `api` client, and use the client created from Context only for cache methods and operation calls:
...
function PetUpdateForm({ petId }: { petId: number }) {
  ...
  const { mutate } = api.pet.updatePet.useMutation(undefined, {
    async onMutate(variables) {
      // ⬇︎ Create a method-only client instance with context options
    ...
    async onError(_error, _variables, context) {
      // Rollback on error
      if (context?.prevPet) {
        createAPIClient(apiOptions!).pet.getPetById.setQueryData(
          petParams,
          context.prevPet
        );
      }
    ...
    async onSuccess() {
      ...
      await createAPIClient(apiOptions!).pet.findPetsByStatus.invalidateQueries();
    },
  });
...
function PetEditor({ petId }: { petId: number }) {
  ...
  // Method-only client that needs direct access to runtime queryClient/requestFn/baseUrl from Context
...
function PetForm({ petId }: { petId: number }) {
  ...
  const { mutate, isPending } = api.pet.updatePet.useMutation(undefined, {
    async onMutate(variables) {
      // Method-only client for cache operations inside callbacks.
      const apiClient = createAPIClient(apiOptions!);
      await apiClient.pet.getPetById.cancelQueries({ parameters: petParams });
      const prevPet = apiClient.pet.getPetById.getQueryData(petParams);
      apiClient.pet.getPetById.setQueryData(petParams, (old) => ({
...
- [CLI Options](./../../openapi/index.mdx) - All available CLI options
- [Redocly Config](./../../cli/redocly-config.mdx) - Configuration file reference
