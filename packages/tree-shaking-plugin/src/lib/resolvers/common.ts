...
/**
 * Ensure that an arbitrary module access object records diagnostic traces for
 * every `resolve` and `load` call.
 *
 * `createQraftModuleAccess` already returns a traceable access object. This
 * helper is for user-provided or test-provided implementations that only expose
 * plain `resolve` and `load` functions. It wraps those functions, delegates the
 * actual work to the original access object, and records whether each delegated
 * call hit, missed, or threw. The collected trace is later read by
 * `getQraftModuleAccessTrace*` helpers and included in diagnostics when source
 * resolution fails.
 *
 * The wrapper does not change resolution or loading semantics. If the access is
 * already traceable, the same object is returned unchanged.
 */
